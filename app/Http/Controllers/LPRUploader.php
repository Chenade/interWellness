...
class LPRUploader extends Controller
{
                ...
                for ($i = 0; $i < $total; $i++) {
                    ...
                    if ($tmpFilePath != '') {
                        ...
                        if (array_key_exists('id', $data) and strlen($data['id'])) {
                            $response = Restful::worker('PUT', '/api/va/bovia/lpr/vehicle', $port, ['id' => $data['id'], 'photo' => $base64], 15, $options);
                            sleep(1);
                        } else $response = response()->json(['error' => trans('api.No_carList')], 400);
                        //error_log(json_encode($response));
                    }
                }
            } else if (array_key_exists('key', $data)) {  // delete

                $response = Restful::worker('PUT', '/api/va/bovia/lpr/vehicle', $port, ['id' => $data['id'], 'photo' => ''], 15, $options);
