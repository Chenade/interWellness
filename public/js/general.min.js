...
$(document).ready(function () {
    var high_cpu_count = 0, high_mem_count = 0, query_server_status = function () {
        if ($('[name="_user"]').attr('content')) {
            ajax('PUT', '/agent/api', data_format('/api/server/list', 0, 'GET', {}, null), function (a) {
                if (!a.error) {
                    for (var i in a) {
                        var site = a[i];
                        for (var j in site) try {
                            var server = site[j];

                            if (server.cpuUsedPercent > 85)
                                high_cpu_count += 1;
                            else
                                high_cpu_count = 0;

                            if (server.memoryAvailable / Math.pow(1024, 3) < 1)
                                high_mem_count += 1;
                            else
                                high_mem_count = 0;

                            if (high_cpu_count >= 3)
                                $.growl.warning({message: langDict.highCpu, duration: 600000});

                            if (high_mem_count >= 3)
                                $.growl.warning({message: langDict.highMem, duration: 600000});

                            if ((server.freeSpace / Math.pow(1024, 3)) < 5)
                                $.growl.warning({message: langDict.highSystemSpace, duration: 600000});

                            for (var srv in server.service) if ('freeSpace' in server.service[srv]) {
                                if (server.service[srv].freeSpace / Math.pow(1024, 3) < (server.keepSpace - ((server.keepSpace / 100) * 10))) {
                                    $.growl.warning({message: langDict.highRecordSpace, duration: 600000});
                                }
                                break;
                            }
                        } catch (e) {
                ...
                setTimeout(query_server_status, 601000);
            });
        }
    };
    ...
    $('#changePassword').on('click', function () {
        var id = $('#user-card-modal').attr('data-id');
        $('#password-modal').attr('data-id', id);
        $('#user-card-modal').modal('hide');
    ...
    $('#password_confirm').on('click', function () {
        var obj = {'id':  $('#password-modal').attr('data-id')};
        if ($('#newPassword').val().length < 8 || $('#newPassword2').val().length < 8) {
    ...
    $('#change2fa').on('click', function () {
        var id = $('#user-card-modal').data('id');
        ajax('PUT', '/agent/api', data_format('/api/user?uid=' + id, 0, 'GET', {}, null), function (user) {
            if(!user.error){
                if(user['2faEnabled']){
                    $('.faArea').css('display', 'none');
                    $('#fa_final').css('display', 'block');
                }else{
                    $('.faArea').css('display', 'none');
                    $('#fa_index').css('display', 'block');
                }
            }else
                $.growl.error({message: user.error});
        });
        $('#user-card-modal').modal('hide');
        $('#2fa-modal').modal({
            backdrop: 'static',
    ...
    $('#2fa_step1').on('click', function () {
        processing = true;
        $('#fa_index').css('display', 'none');
        $('#fa_qrcode').css('display', 'block');
        ajax('PUT', '/agent/api', data_format('/api/user/2fa/generate', 0, 'POST', {}, null), function (a) {
            if (!a.error){
                $('#faQRCode').attr('src', 'data:image/png;base64,'+ a['2faImage']);
            }else
                $.growl.error({message: a.error});
        });
    });
    $('#2fa_step2').on('click', function () {
        $('#fa_qrcode').css('display', 'none');
        $('#fa_input').css('display', 'block');
    });
    $('#2fa_step3').on('click', function () {
        var code = "";
        $('.twoFaCode').each(function(i,element){
            code += $(element).val();
        });

        ajax('PUT', '/agent/api', data_format('/api/user/2fa/enable', 0, 'POST', {'code': code}, null), function (a) {
            if (!a.error){
                processing = false;
                $('.twoFaCode').val('');
                $('#fa_input').css('display', 'none');
                $('#fa_final').css('display', 'block');
                $.growl.notice({message: langDict.wasActive});
            }else
                $.growl.error({message: a.error});
        });
    });
    $('#2fa_disable').on('click', function () {
        var id = $('#user-card-modal').data('id');
        ...
        ajax('PUT', '/agent/api', data_format('/api/user', 0, 'PUT', obj, null), function (a) {
            if (!a.error){
                ...
                $('#fa_final').css('display', 'none');
                $('#fa_index').css('display', 'block');
            }else
                $.growl.error({message: a.error});
        });
    });
    $('#2fa-modal').on('hidden.bs.modal', function (e) {
        if(processing) $.growl.error({message: langDict.activeFailed});
        processing = false;
        $('#user-card-modal').modal({
            backdrop: 'static',
            keyboard: true,
    ...
    $('.twoFaCode').on('keyup keydown click', function (e) {
        switch (e.type) {
            case "keyup":
                var key = e.which,
                    t = $(e.target),
                    sib = t.closest('div').next().find('.twoFaCode');
                ...
                if (!sib || !sib.length) sib = $('.twoFaCode').eq(0);
                sib.select().focus();
                break;
            case "keydown":
                var key = e.which;
                if (key === 13) $('#2fa_step3').click();
